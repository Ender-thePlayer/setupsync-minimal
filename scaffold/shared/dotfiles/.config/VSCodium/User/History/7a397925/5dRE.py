...
def on_download_button_clicked(parent_window, config_file, youtube_link_entry, spotify_link_entry, button):
    ...
    if not client_id == "" and not client_secret == "" and not spotify_link_entry_text == "":
        command = f"""
        /home/eepyender/.local/bin/yt-dlp -o "{folder_path}/{spotify_id}.%(ext)s" -x -f bestaudio --extract-audio --audio-quality 0 --audio-format mp3 {youtube_link_entry_text}
        """

    else:
        command = f"""
        /home/eepyender/.local/bin/yt-dlp -o "{folder_path}/%(title)s.%(ext)s" --embed-metadata --embed-thumbnail -v --convert-thumbnail jpg --ppa "EmbedThumbnail+ffmpeg_o:-c:v mjpeg -vf crop=\\"'if(gt(ih,iw),iw,ih)':'if(gt(iw,ih),ih,iw)'\\"" --exec ffprobe -x -f bestaudio --extract-audio --audio-quality 0 --audio-format mp3 {youtube_link_entry_text}
        """
